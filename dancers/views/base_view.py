...
r = edis.Redis(os.environ.get("REDIS_URL"))
...
class BaseView(View):
            ...
            def wrapper(*args, **kwargs):
                ...
                if access_token:
                    ...
                    if result['data']['type'] == 'User':
                        ...
                        if result['data']['from_service'] != from_service or result['data']['to_service'] != to_service:
                            return {'status': 'Failed', 'message': 'Bad request', 'code': 400}
    ...
    def my_authorization(service):
        ...
        if access_token:
            ...
            response = requests.post(settings.AUTH_SERVICE_BASE_URL + 'services/auth/',
                                     json={'refresh_token': refresh_token.decode('UTF-8')})
            data = response.json()
    ...
    def send_stat_log(request, client, client_id, url=settings.STATISTIC_SERVICE_BASE_URL):
        ...
        headers = {'Service': 'Dancers'}

        try:
            response = requests.post(url + 'statistics/logs/', json=data, timeout=3, headers=headers)
    ...
    @staticmethod
    def send_stat_error(request, error, url=settings.STATISTIC_SERVICE_BASE_URL):
        data = {
            'time': '',
            'error': error,
        ...
        try:
            response = requests.post(url + 'statistics/dancers/errors/', json=data, timeout=3)
            if response.status_code != 200:
                return False
            return True
        except Timeout:
